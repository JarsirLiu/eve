...
## Request maintenance compaction

When a conversation is parked after a turn, request compaction without adding a user message or starting another model turn:
...
const compacted = await session.compact({ commandId: "compact-2026-01-01" });
console.log(compacted.status, compacted.commandId);

for await (const event of session.stream()) {
  if (event.type === "compaction.completed") {
    console.log("compaction changed history:", event.data.changed);
    break;
  }
...
`compact()` returns after the request is admitted. It is valid for parked conversation sessions; an active turn is rejected with `409`, and no new session is created. Observe the event stream for completion or failure. Manual compaction bypasses the automatic threshold check but uses the same compaction policy and durable state preservation. A successful no-op reports `changed: false`.
