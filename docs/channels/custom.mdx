...
- `getSession(sessionId)` looks up an existing session. The returned `Session` exposes `getEventStream({ startIndex? })` for streaming, `cancel({ turnId? })` for session-id-addressed cancellation, and `compact({ commandId? })` for maintenance compaction.
...
Manual compaction is a maintenance request for a parked conversation session. It does not send a message or create a turn:

```ts
POST("/threads/:threadId/compact", async (_req, { getSession, params }) => {
  const session = getSession(params.threadId);
  const result = await session.compact({ commandId: "compact-2026-01-01" });
  return Response.json(result, { status: 202 });
});
```

The returned status means that the command was accepted. Read the session event stream for `compaction.completed` or `compaction.failed`. Manual compaction bypasses the automatic threshold, preserves framework-owned session state, and leaves the session parked. An active turn is rejected rather than cancelled or queued; retries with the same `commandId` are idempotent.
