...
The application exposes a health route plus eve channel routes that inspect the agent, create sessions, send follow-ups, request maintenance compaction, cancel turns, and stream events:

- `GET /eve/v1/health`
- `GET /eve/v1/info`
- `POST /eve/v1/session` (start a session)
- `POST /eve/v1/session/:sessionId` (send a follow-up)
- `POST /eve/v1/session/:sessionId/compact` (compact a parked session)
...
Request maintenance compaction after the session reaches `session.waiting`:

```bash
curl -X POST https://<deployment>/eve/v1/session/ses_01h.../compact \
  -H "Content-Type: application/json" \
  -d '{"commandId":"compact-2026-01-01"}'
# {"commandId":"compact-2026-01-01","ok":true,"sessionId":"ses_01h...","status":"accepted"}
```

The route accepts parked conversation sessions only. It does not create a model turn or add a message to the conversation; an active turn returns `409`. The acknowledgement only confirms admission, so observe `compaction.completed` or `compaction.failed` on the stream. Manual compaction ignores the automatic threshold and uses the existing compaction policy. A no-op completes with `changed: false`.
